...
從程式碼中可以看到，`<title>`及`<div class="page">`2個標籤中，都有一個`{% block  %}{% endblock %}`的樣式的程式碼，所以當`page.html`中使用`{% extends "layout.html" %}`，`page.html`就可以使用`layout.html`的版面，並設定這2個block裡面的內容。也就是說，當Lektor產生靜態網頁時，在page的這個頁面中，會使用layout.html設定的畫面，並根據page.html設定的block，使頁面中指定的位置出現我們要的資訊，可以想像是layout.html有title這個凹槽，而page.html設定這個凹槽要放的東西。
...
由於page.html是預設的頁面樣式，若content中沒有指定model，就會指定預設`_model: page`。下面用quickstart中，about的頁面來說明：
...
1. 首先在`content`資料夾中找到`about/contents.lr`檔案，
    ![tree view on about](/assets/螢幕快照%202019-09-14%20下午3.15.16.png)
...
3. page.html中使用`{% extends "layout.html" %}`，所以將以`layout.html`為框架建立網頁。
4. 從page.html設定block title及block body。
...
7. 生成網頁原始碼如下：
    ...
    ```
8. 生成畫面如下：
    ![about page](/assets/螢幕快照%202019-09-14%20下午3.30.23.png)